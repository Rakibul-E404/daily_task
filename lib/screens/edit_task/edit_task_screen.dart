...
class _EditTaskScreenState extends State<EditTaskScreen> {
  late final EditTaskController _editController;

  late TextEditingController _titleController;
  late TextEditingController _descriptionController;

  /// Working copy — existing subtasks keep their server id; new ones have null id.
  ...
  void initState() {
    super.initState();
    _editController = Get.put(EditTaskController());
    ...
    _selectedDateTime = widget.originalTask.startTime ?? DateTime.now();
    _selectedStatus   = widget.originalTask.status;

    // Normalize priority: lowercase + fallback to 'medium' if null/unrecognized.
    // Guards against API returning null, '', or mixed-case like 'Medium'.
    const validPriorities = {'low', 'medium', 'high'};
    final rawPriority = widget.originalTask.priority.trim().toLowerCase();
    _selectedPriority = validPriorities.contains(rawPriority) ? rawPriority : 'medium';

    // Preserve server ids so the diff in the controller works correctly
    ...
    _subtaskFocusNodes = _editableSubTasks.map((_) {
      ...
      // Rebuild whenever focus changes so the trash/check icon stays in sync
  ...
  void _confirmDeleteSubtask(int index) {
    // Capture the subtask reference by value — safe even if list mutates later
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: AppTextStyles.defaultTextStyle.copyWith(color: AppColors.grey),
            ),
  ...
  void _addNewSubtask() {
    final node = FocusNode();
    // New node also needs the focus listener
    node.addListener(() => setState(() {}));
  ...
  /// Pass the subtask object explicitly so the correct id is tracked
  /// even if the list has shifted by the time this runs.
  void _removeSubtask(int index, SubTask subtask) {
    if (subtask.id != null && subtask.id!.isNotEmpty) {
      _deletedSubtaskIds.add(subtask.id!);
      debugPrint('🗑️ Queued for deletion: ${subtask.id}  (total: ${_deletedSubtaskIds.length})');
    }
    setState(() {
  ...
  Future<void> _saveTask() async {
    ...
    final currentSubtasks = _editableSubTasks.where((sub) {
      final isNew = sub.id == null || sub.id!.isEmpty;
      return !isNew || sub.title.isNotEmpty;
    }).toList();

    debugPrint('📤 deletedSubtaskIds at save: $_deletedSubtaskIds');
    ...
    if (success) {
      ...
      // Pop with a simple flag — TaskDetailsScreen will re-fetch from server,
      // which is the only reliable source of truth (especially for deletions).
          ...
          child: Column(
            ...
            children: [
              ...
              ...List.generate(_editableSubTasks.length, (index) {
                ...
                // Show ✓ only when the field is actively focused AND has text.
                // In every other state (unfocused, empty, just loaded) show 🗑️.
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            ...
                            // ── Trash / Save icon ──────────────────
                            // Reads focus directly from state (not from
                            // ValueListenableBuilder) so it's always current.
                            IconButton(
                              onPressed: () {
                                if (showSave) {
                                  _saveSubtask(index);
              ...
              Obx(
                  ...
                  child: ElevatedButton(
                    onPressed:
                    _editController.isUpdating.value ? null : _saveTask,
                    style: ElevatedButton.styleFrom(
                      backgroundColor: AppColors.primaryColor,
