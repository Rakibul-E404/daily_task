...
class ManualSignUpController extends GetxController {
  ...
  // ✅ FIXED: Keep date in DD/MM/YYYY format (not converting to ISO)
  String _formatDateForApi(String date) {
    ...
    // Check if date is in DD-MM-YYYY format (convert to DD/MM/YYYY)
    if (cleanDate.contains('-')) {
      final parts = cleanDate.split('-');
      if (parts.length == 3) {
        final day = parts[0].padLeft(2, '0');
  ...
  Future<bool> signUp({int retryCount = 0}) async {
    ...
    try {
      ...
      final formattedDob = _formatDateForApi(dateOfBirth.value);

      // ✅ FIXED: Prepare request body with correct field names
      final Map<String, dynamic> requestBody = {
        "name": name.value.trim(),
        "email": email.value.trim(),
        "password": password.value.trim(),
        ...
        "role": "individual",
        "acceptTOC": true,
        "age": age.value.trim(),           // ✅ Send as String
        "dob": formattedDob,               // ✅ Changed from "date" to "dob"
        "gender": gender.value.toLowerCase(),
      };
