...
class _VerifyEmailScreenState extends State<VerifyEmailScreen> {
  ...
  void _extractArguments() {
    ...
    if (arguments != null) {
      if (arguments is Map) {
        // Handle both key names: 'verificationToken' and 'resetToken'
        _verificationToken = arguments['verificationToken'] as String? ??
  ...
  Future<void> _getTokenFromStorage() async {
    try {
      final token = await SecureStorageService.instance.getTemporaryData('reset_password_token');
      if (token != null && token.isNotEmpty) {
        setState(() {
          _verificationToken = token;
        });
        print('✅ Retrieved token from storage: $_verificationToken');
      }
    } catch (e) {
  ...
  Future<void> _verifyAndNavigate() async {
    ...
    try {
      // Determine which API to use based on auth flow
      final isForgotPassword = _authFlow == AuthFlowModel.forgotPassword;
      // Use verifyEmail API for both cases, the backend handles it differently
      ...
      // Add token for forgot password flow
      if (isForgotPassword && _verificationToken != null && _verificationToken!.isNotEmpty) {
        requestBody["token"] = _verificationToken;
      }
      ...
      if (response.isSuccess) {
        ...
        if (_authFlow == AuthFlowModel.forgotPassword) {
          // Pass email and OTP to SetNewPasswordScreen
          Get.off(() => SetNewPasswordScreen(
            userEmail: _userEmail,
            otp: pinController.text, // Pass the 6-digit OTP entered by user
          ));
        }
