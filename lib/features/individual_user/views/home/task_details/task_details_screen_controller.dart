...
class TaskDetailsScreenController extends GetxController {
  ...
  void _getTaskIdAndFetch() {
    ...
    if (arguments == null) {
      ...
      print('Error: No arguments passed to TaskDetailsScreen');
      return;
    }
    ...
    if (arguments is String) {
      ...
      // If Task object is passed directly
...
class TaskDetailsScreenController extends GetxController {
  ...
  Future<void> fetchTaskDetails(String taskId) async {
    ...
    try {
      ...
      if (response.isSuccess && response.jsonResponse != null) {
        ...
        // Debug: Print the raw subtasks data
        print('🔍 Raw subtasks data: ${attributes?['subtasks']}');
  ...
  // ✅ FIXED: Now uses subtaskId correctly
  Future<void> toggleSubtaskStatus(String taskId, String? subtaskId, bool isCompleted, int subtaskIndex) async {
    // ✅ Add null check
    if (subtaskId == null || subtaskId.isEmpty) {
      print('❌ Subtask ID is null or empty!');
      ScaffoldMessenger.of(Get.context!).showSnackBar(
        const SnackBar(
  ...
  Task _mapApiResponseToTask(Map<String, dynamic> json) {
    ...
    if (json['subtasks'] != null && json['subtasks'] is List) {
      subtasks = (json['subtasks'] as List).map((subtaskJson) {
        print('🔍 Subtask JSON: $subtaskJson'); // Debug print

        return SubTask(
          id: subtaskJson['_id']?.toString() ?? '', // ✅ Extract _id
          title: subtaskJson['title'] ?? '',
          isCompleted: subtaskJson['isCompleted'] ?? false,
          duration: subtaskJson['duration']?.toString(),
        );
    ...
    print('✅ Parsed ${subtasks.length} subtasks');
    for (var st in subtasks) {
      print('   Subtask: id=${st.id}, title=${st.title}, completed=${st.isCompleted}');
    }
