...
class TaskDetailsScreenController extends GetxController {
    ...
    try {
      ...
      if (response.isSuccess || response.statusCode == 200) {
        ...
        if (currentTask != null) {
          ...
          final int newCompletedCount = updatedSubtasks
  ...
  void _checkSubtaskMilestone({
    ...
    if (completedCount == (totalCount / 2).ceil() && !_shown50Milestone) {
      ...
      final mode = creativeMode.value.isNotEmpty
          ? creativeMode.value
          : 'encouraging';
  ...
  SupportAlertType _getAlertTypeForMilestone(String milestone, String mode) {
    ...
    switch (mode) {
      ...
      case 'logical':
        return isHalf
            ? SupportAlertType.logical50
            : SupportAlertType.logical100;
      case 'encouraging':
      default:
        return isHalf
  ...
  void checkMilestoneAfterBulkComplete(BuildContext context, Task currentTask) {
    final int completedCount = currentTask.subtasks
        .where((s) => s.isCompleted)
        .length;
  ...
  Future<bool> updateTaskStatus(
    ...
    try {
      ...
      if (response.isSuccess || response.statusCode == 200) {
        ...
        final hasCreativeResponse = _parseCreativeResponse(
    ...
    try {
      ...
      if (creativeResponse != null) {
        ...
        if (popup != null) {
          ...
          print(
            '🎉 Creative Response - Mode: ${creativeMode.value}, Milestone: ${creativeMilestone.value}',
          );
  ...
  SupportAlertType? _getAlertTypeFromCreativeResponse() {
    ...
    print(
      '⚠️ No matching alert type found for milestone: $milestone, mode: $mode',
    );
    return null;
  }
  ...
  Task _mapApiResponseToTask(Map<String, dynamic> json) {
    ...
    if (json['subtasks'] != null && json['subtasks'] is List) {
      subtasks = (json['subtasks'] as List).map((subtaskJson) {
        final subId =
            subtaskJson['_subTaskId']?.toString() ?? // ← primary key
            subtaskJson['_id']?.toString() ?? // ← fallback
            '';
        return SubTask(
