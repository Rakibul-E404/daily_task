...
class _EditTaskScreenState extends State<EditTaskScreen> {
  ...
  void _saveTask() {
    ...
    // Update all subtasks with latest titles
    for (int i = 0; i < _editableSubTasks.length; i++) {
      _editableSubTasks[i] = SubTask(
        title: _subtaskControllers[i].text.trim(),
        isCompleted: _editableSubTasks[i].isCompleted,
    ...
    final validSubtasks = _editableSubTasks
        .where((sub) => sub.title.isNotEmpty)
        .toList();
    ...
    final updatedTask = widget.originalTask.copyWith(
      ...
      totalSubtasks: validSubtasks.length,
      completedSubtasks: validSubtasks.where((s) => s.isCompleted).length,
    );
