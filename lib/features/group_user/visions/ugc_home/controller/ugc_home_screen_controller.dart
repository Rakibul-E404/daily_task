...
class UgcHomeController extends GetxController {
  ...
  UgcTask _mapApiResponseToUgcTask(Map<String, dynamic> json) {
    ...
    // );
    // In _mapApiResponseToUgcTask method, add:
    return UgcTask(
      id: json['_id'] ?? '',
      taskType: taskType,
      title: json['title'] ?? '',
...
class UgcHomeController extends GetxController {
  ...
  UgcTask _mapApiResponseToUgcTask(Map<String, dynamic> json) {
    ...
    if (subtasksList != null) {
      for (var subtaskJson in subtasksList) {
        // Check if myCompletion exists (for collaborative tasks)
        final myCompletion = subtaskJson['myCompletion'] ?? {};
        ugcSubtasks.add(
          UgcSubTask(
            id: subtaskJson['_id'] ?? '',
            title: subtaskJson['title'] ?? '',
            isCompleted: myCompletion['isCompleted'] ?? subtaskJson['isCompleted'] ?? false,
