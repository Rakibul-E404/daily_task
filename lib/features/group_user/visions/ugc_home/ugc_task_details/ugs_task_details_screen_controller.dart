...
///
///
/// todo::: showign the support allert
///
///
///
///
...
//   }
//
//   // Parse creative response from API
...
//           print('💬 Title: ${creativePopupTitle.value}');
//           print('💬 Message: ${creativePopupMessage.value}');
//
//           // Determine alert type immediately
...
//   }
//
//   // Get alert type from creative response
...
//
//     // Started milestone - 50% completion
//     if (milestone == 'started') {
//       if (mode == 'calm') {
//         return SupportAlertType.calm50;
//       } else if (mode == 'encouraging') {
//         return SupportAlertType.encouraging50;
//       } else if (mode == 'logical') {
//         return SupportAlertType.logical50;
//       }
//     }
//
//     // Halfway milestone - 50% completion
//     if (milestone == '1/2') {
//       if (mode == 'calm') {
//         return SupportAlertType.calm50;
//       } else if (mode == 'encouraging') {
...
//     }
//
//     // Completed milestone - 100% completion
//     if (milestone == '100') {
//       if (mode == 'calm') {
//         return SupportAlertType.calm100;
//       } else if (mode == 'encouraging') {
//         return SupportAlertType.encouraging100;
//       } else if (mode == 'logical') {
//         return SupportAlertType.logical100;
//       }
//     }
//
...
//   }
//
//   // Show support alert based on creative response
...
//
//     // Use a delayed execution to ensure the dialog shows after any existing dialogs are closed
...
//             // Reset flags after alert is dismissed
...
class UgcTaskDetailsController extends GetxController {
  ...
  Future<bool> updatePersonalTaskStatus(String taskId, String status) async {
    ...
    try {
      ...
      if (response.isSuccess || response.statusCode == 200) {
        // Parse creative response (may be null for personal tasks)
        final hasCreativeResponse = _parseCreativeResponse(response.jsonResponse);

        await fetchTaskDetails(taskId);
        isUpdatingStatus.value = false;

        // If no creative response from API, create a fallback alert based on status
  ...
  // Create fallback alert when API doesn't provide creative response
  void _createFallbackAlert(String newStatus) {
    ...
    if (newStatus == 'inProgress') {
      // Task started - 50% milestone
      alertType = SupportAlertType.encouraging50;
    } else if (newStatus == 'completed') {
      // Task completed - 100% milestone
  ...
  // Parse creative response from API - returns true if creative response exists
  ...
  // Get alert type from creative response - FIXED for '100_percent' milestone
  SupportAlertType? _getAlertTypeFromCreativeResponse() {
    final milestone = creativeMilestone.value;
    final mode = creativeMode.value;
    ...
    // Check for different milestone formats for halfway/started
    ...
    // Check for different milestone formats for completed
    ...
    // Started or Halfway milestone - 50% completion
    if (isHalfway) {
      if (mode == 'calm') {
        return SupportAlertType.calm50;
      } else if (mode == 'encouraging') {
      ...
      // Default to encouraging if mode is unknown
      return SupportAlertType.encouraging50;
    }
  ...
  // Show support alert based on creative response or fallback
  void showSupportAlertIfNeeded(BuildContext context) {
    print('🔔 showSupportAlertIfNeeded called');
    print('📱 showCreativePopup.value: ${showCreativePopup.value}');
    print('🎯 pendingAlertType.value: ${pendingAlertType.value}');
