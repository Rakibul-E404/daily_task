...
///
///
/// todo:: adding edit screen functionality
///
///
///
///
...
import 'package:get/get.dart';
import 'package:intl/intl.dart';
import '../../../../../screens/edit_task/ugc_edit_task_screen.dart';
import '../../../../../utils/app_colors.dart';
import '../../../../../utils/app_texts_style.dart';
import '../../bottom_navigation/ugc_bottom_nav.dart';
import 'ugs_task_details_screen_controller.dart';
...
class UgcTaskDetailsScreen extends StatelessWidget {
  ...
  Future<void> _handleStatusUpdate(
    ...
    if (task.status == TaskStatus.pending) {
      // START task
      final success =
      await controller.updateTaskStatus(task.id, 'inProgress');
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Task started successfully!'),
      ...
      if (incompleteSubtasks.isNotEmpty) {
        ...
        if (currentTask != null) {
          final allDone = currentTask.subtasks.every((s) => s.isCompleted);
          if (allDone) {
            controller.checkMilestoneAfterBulkComplete(
                context, currentTask);
          }
        }
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
                'All subtasks completed! Tap Complete again to finish.'),
            backgroundColor: Colors.green,
            duration: Duration(seconds: 2),
          ),
        );
      ...
      appBar: AppBar(
        ...
        actions: [
          Obx(() {
            ...
            // Only show the ⋯ menu for personal tasks that are not completed
            if (!isPersonalTask || task == null) {
              return const SizedBox.shrink();
            }
                    ...
                    child: InkWell(
                      ...
                      onTap: () async {
                        ...
                        await showMenu<String>(
                          ...
                          if (value == 'edit') {
                            // ✅ Navigate to UgcEditTaskScreen and
                            // re-fetch from server on successful return
                            Get.to(() =>
                                UgcEditTaskScreen(originalTask: task))
                                ?.then((result) {
                              if (result == true) {
                                final taskId =
  ...
  Widget _buildContent(
    ...
    // Subtasks can only be toggled when task is IN PROGRESS
    final isCheckboxEnabled =
        isInProgress && !controller.isTogglingSubtask.value;
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.info_outline,
                          color:
                          AppColors.notice.withValues(alpha: 0.7),
                          size: 20),
                      const SizedBox(width: 8),
                      Expanded(
                        child: Text(
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    if (controller.isTogglingSubtask.value &&
                        controller
                            .currentTogglingSubtaskIndex.value ==
                            index)
                      const SizedBox(
                        width: 20,
                        height: 20,
