...
class UgcTaskDetailsScreen extends StatelessWidget {
  ...
  Future<void> _handleStatusUpdate(BuildContext context, UgcTaskDetailsController controller, UgcTask task) async {
    ...
    if (task.status == TaskStatus.pending) {
      ...
      final incompleteSubtasks = task.subtasks
      ...
      if (incompleteSubtasks.isNotEmpty) {
        ...
        await controller.fetchTaskDetails(task.id);

        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('All subtasks completed! Tap Complete again to finish.'),
            backgroundColor: Colors.green,
            duration: Duration(seconds: 2),
          ),
        );
        ...
        final success = await controller.updateTaskStatus(task.id, 'completed');
  ...
  Widget _buildContent(BuildContext context, UgcTask task, UgcTaskDetailsController controller) {
    ...
    final isCheckboxEnabled = isInProgress && !controller.isTogglingSubtask.value;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (task.subtasks.isNotEmpty)
              _buildSubTaskList(task, controller, isCheckboxEnabled),
            const SizedBox(height: 16),
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          "Click 'Start' to complete the sub-tasks.",
                          style: TextStyle(
                            fontSize: 12,
            ...
            /// Message when all subtasks are completed (only for inProgress tasks)
            if (isInProgress && allSubtasksCompleted && task.subtasks.isNotEmpty)
              Padding(
                padding: const EdgeInsets.only(bottom: 16.0),
                child: Container(
            ...
            /// Message when there are incomplete subtasks (only for inProgress tasks)
            if (isInProgress && !allSubtasksCompleted && task.subtasks.isNotEmpty)
              Padding(
                padding: const EdgeInsets.only(bottom: 16.0),
                child: Text(
            ...
            if (!isCompleted)
              SizedBox(
                ...
                child: Obx(() {
                  ...
                  bool isButtonEnabled = !controller.isUpdatingStatus.value;

                  // For inProgress tasks, button is always enabled (it will auto-complete subtasks)
                  if (isInProgress) {
  ...
  Widget _buildSubTaskList(UgcTask task, UgcTaskDetailsController controller, bool isCheckboxEnabled) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
              ...
              child: Padding(
                ...
                child: Row(
                  children: [
                    ...
                    GestureDetector(
                      behavior: HitTestBehavior.opaque, // 🔥 This fixes the tap issue!
...
///
///
/// TODo::DFIXONG THE CASE OF SUPPORT
///
///
///
...
class UgcTaskDetailsScreen extends StatelessWidget {
  ...
  void _confirmDeleteTask(
      ...
      UgcTaskDetailsController controller,
      String taskId,
      ) {
    showModalBottomSheet(
      context: context,
  ...
  Future<void> _handleStatusUpdate(
    ...
    if (task.status == TaskStatus.pending) {
      ...
      if (incompleteSubtasks.isNotEmpty) {
        ...
        // ✅ After auto-completing all subtasks, check 100% milestone
        // (individual taps already handled 50%, this covers the bulk finish)
        final currentTask = controller.task.value;
        if (currentTask != null) {
          final allDone = currentTask.subtasks.every((s) => s.isCompleted);
          if (allDone) {
            controller.checkMilestoneAfterBulkComplete(context, currentTask);
          }
        }
        ...
        return; // User taps Complete again to finish
      } else {
  ...
  Widget _buildContent(
    ...
    // Subtasks can only be checked when task is IN PROGRESS
    final isCheckboxEnabled =
        isInProgress && !controller.isTogglingSubtask.value;
          ...
          children: [
            CircleAvatar(
              ...
              backgroundImage: task.assignedByImage != null &&
                  ...
                  ? NetworkImage(task.assignedByImage!)
                  : const AssetImage("assets/images/dummy_user_image.png")
              as ImageProvider,
              onBackgroundImageError: (_, __) {},
            ),
            const SizedBox(width: 12),
