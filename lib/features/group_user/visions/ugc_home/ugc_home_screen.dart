...
class _UgcHomeScreenState extends State<UgcHomeScreen> {
  ...
  // Adjust this value to control how hard you need to pull
  // Higher = harder to trigger, Lower = easier to trigger
  final double _refreshThreshold = 150.0; // Changed from 80 to 120 (harder to trigger)
  ...
  Future<void> _performSilentRefresh() async {
    ...
    print('🔄 Refresh triggered at ${_pullDistance}px');
    _isRefreshing = true;
        ...
        body: NotificationListener<ScrollNotification>(
          onNotification: (notification) {
            ...
            if (notification is ScrollUpdateNotification) {
              ...
              if (notification.metrics.pixels <= 0 && notification.metrics.extentBefore == 0) {
                // Calculate pull distance (how far user has pulled down)
                // The more negative the pixels, the further the pull
                final pullOffset = notification.metrics.pixels.abs();
                _pullDistance = pullOffset;

                print('Pull distance: $_pullDistance px / Threshold: $_refreshThreshold px');

                // Trigger refresh only when pull distance exceeds threshold
                if (_pullDistance >= _refreshThreshold && !_hasTriggered && !_isRefreshing) {
...
class _UgcHomeScreenState extends State<UgcHomeScreen> {
  ...
  void initState() {
    ...
    // ✅ FIX: Create ScrollController once and attach pagination listener here
  ...
  @override
  void dispose() {
    _scrollController.dispose(); // ✅ Properly dispose
    Get.delete<UgcHomeController>();
    super.dispose();
  }
    ...
    return Obx(() {
      ...
      return Scaffold(
        ...
        body: NotificationListener<ScrollNotification>(
          ...
          child: CustomScrollView(
            // ✅ FIX: Use the persistent ScrollController (not a new one every build)
  ...
  Widget _buildTasksContent(
    ...
    return SliverPadding(
      ...
      sliver: SliverList(
        delegate: SliverChildBuilderDelegate(
              (context, index) {
            // ✅ FIX: Removed addPostFrameCallback pagination trigger here.
            // Pagination is now handled solely by the ScrollController listener
            // in initState(), which prevents duplicate API calls.
